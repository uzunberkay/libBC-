...
int main()
{
 ...
 /*   uint16_t  ival = 0xFFAB;
    uint8_t  ivalLow = (uint8_t)ival & 0xFF;
    uint8_t  ivalHigh = (uint8_t)(ival >> 8) & 0xFF;

    printf(" Ival : %#X  Low : %#X  High : %#X", ival, ivalLow, ivalHigh);*/

    uint8_t pinNumber = 12;

 /*   for (size_t pinNumber = 0; pinNumber < 16; pinNumber++)
    {
        if (!(pinNumber & 0x07))
        {
            printf("pinNumber : %lld \n",pinNumber);
        }
    }*/

    //printf("%d: %d and %d", pinNumber, pinNumber / 4 ,(pinNumber % 4) * 4 );

    //uint8_t  a = -32;   
    //printf("\n%zu", a);

    uint32_t variable[2] = { 1,4 };
    uint32_t* pVariable = variable;

    uint32_t temp = (uint32_t)pVariable;
    temp += 0x4;
    *((uint32_t*)temp) = 22;
    printf("%d %d ", variable[0], variable[1]);
